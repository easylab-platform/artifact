...
func (s *State) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case path == "api/v1/api_key":
		artifactkit.JSON(w, http.StatusOK, map[string]any{"rubygems_api_key": "test-token", "name": "test"})
...
func (s *State) names(w http.ResponseWriter, r *http.Request) {
	...
	if remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "rubygems", Sub: "index"}); err == nil {
...
func (s *State) versionsAPI(w http.ResponseWriter, r *http.Request, name string) {
	...
	if len(versions) == 0 {
		remote, _ := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "rubygems"})
		if remote != nil {
...
func (s *State) latest(w http.ResponseWriter, r *http.Request, name string) {
	versions, _ := s.Registry.Meta.ListVersions(r.Context(), "rubygems", name)
	artifactkit.JSON(w, http.StatusOK, map[string]any{"version": artifactkit.HighestVersion(versions)})
}
...
func (s *State) owners(w http.ResponseWriter, r *http.Request) {
	artifactkit.JSON(w, http.StatusOK, []any{map[string]any{"id": 1, "handle": "anonymous", "role": "owner"}})
}
...
func (s *State) searchGem(w http.ResponseWriter, r *http.Request) {
	...
	if remote, _ := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "rubygems"}); remote != nil {
...
func (s *State) versionV2(w http.ResponseWriter, r *http.Request, path string) {
	...
	if len(parts) >= 3 {
		...
		artifactkit.JSON(w, http.StatusOK, map[string]any{"name": name, "version": ver, "platform": "ruby", "number": ver, "created_at": "2024-01-01T00:00:00Z"})
		return
	}
	w.WriteHeader(http.StatusNotFound)
}
...
func (s *State) compactVersions(w http.ResponseWriter, r *http.Request) {
	...
	artifactkit.Text(w, http.StatusOK, sb.String(), "text/plain; version=1")
}
...
func (s *State) compactInfo(w http.ResponseWriter, r *http.Request, name string) {
	...
	artifactkit.Text(w, http.StatusOK, sb.String(), "text/plain")
}
...
func (s *State) download(w http.ResponseWriter, r *http.Request, filename string) {
	...
	if remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "rubygems", Sub: "gems"}); err == nil {
...
func yankedSet(ctx context.Context, reg *artifactkit.Registry) map[string]bool {
	set := map[string]bool{}
...
func storeVersionSource(reg *artifactkit.Registry, name, version, filename string, data []byte, source string, ctx context.Context) {
	art := artifactkit.Artifact{Format: "rubygems", Repository: name, Version: version, Source: source}
	if len(data) > 0 {
