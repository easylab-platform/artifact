...
func TestHostedUploadAndIndex(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPut, "/artifacts/debian/easylab/easylab-worker_1.0.0_amd64.deb", bytes.NewReader(deb))
	s.ServeHTTP(rec, req)
	...
	req2 := httptest.NewRequest(http.MethodGet, "/artifacts/debian/easylab/Packages", nil)
	...
	req3 := httptest.NewRequest(http.MethodGet, "/artifacts/debian/easylab/easylab-worker_1.0.0_amd64.deb", nil)
	...
	s.ServeHTTP(rec4, httptest.NewRequest(http.MethodDelete, "/artifacts/debian/easylab/easylab-worker_1.0.0_amd64.deb", nil))
	if rec4.Code != http.StatusNoContent {
		t.Fatalf("delete: %d", rec4.Code)
	}
	rec5 := httptest.NewRecorder()
	s.ServeHTTP(rec5, httptest.NewRequest(http.MethodGet, "/artifacts/debian/easylab/Packages", nil))
