...
func (s *State) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case path == "/v3/index.json":
		s.serviceIndex(w, r)
	case strings.HasPrefix(path, "/v3/query"), path == "/query":
		s.search(w, r)
	case strings.HasPrefix(path, "/v3/autocomplete"), path == "/autocomplete":
		s.autocomplete(w, r)
...
func (s *State) serviceIndex(w http.ResponseWriter, r *http.Request) {
	...
	artifactkit.JSON(w, http.StatusOK, map[string]any{
		"version": "3.0.0",
		"resources": []any{
			// The search/autocomplete services live on nuget.org's search
			// host, not the registration host; that URL is what real clients
			// expect (the sidecar maps it back here). Serving them under the
			// registration origin makes NuGet report "no Search service".
			map[string]any{"@id": "https://azuresearch-usnc.nuget.org/query", "@type": "SearchQueryService"},
			map[string]any{"@id": "https://azuresearch-usnc.nuget.org/query", "@type": "SearchQueryService/3.0.0-beta"},
			map[string]any{"@id": "https://azuresearch-usnc.nuget.org/query", "@type": "SearchQueryService/3.5.0"},
			map[string]any{"@id": "https://azuresearch-usnc.nuget.org/autocomplete", "@type": "SearchAutocompleteService"},
			map[string]any{"@id": "https://azuresearch-usnc.nuget.org/autocomplete", "@type": "SearchAutocompleteService/3.5.0"},
