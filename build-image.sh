#!/usr/bin/env bash
# Build and push the artifact registry image WITHOUT a local build daemon.
#
# Pipeline (same verified shape as easylab/build-image.sh):
#   1. buildctl targets the shared cluster buildkitd, builds the Dockerfile
#      with the artifact repo root as context, and exports a docker archive
#      (RepoTag set). buildkitd does NOT push.
#   2. skopeo copies the archive to the forgejo OCI registry (the only
...
# Context is the repo root (go.work + all protocol modules + cmd/). Exclude the
# local data dir and VCS metadata to keep the context small.
CTX="${WORK}/ctx"
mkdir -p "${CTX}"
tar -C "${SRC_DIR}" \
  --exclude='./.git' --exclude='./data' --exclude='./*.tar' \
  -cf - . | tar -C "${CTX}" -xf -

echo "Building artifact image -> ${DEST} (buildkitd=${BUILDKIT})"
