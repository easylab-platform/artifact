...
func init() { artifactkit.Register("composer", NewHandler) }

func (s *State) base() string { return artifactkit.SelfURL(s.SelfBase, "composer") }
...
func (s *State) listJSON(w http.ResponseWriter, r *http.Request) {
	...
	artifactkit.JSON(w, http.StatusOK, map[string]any{"packageNames": names})
}
...
func (s *State) search(w http.ResponseWriter, r *http.Request) {
	...
	if remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "composer", Sub: "search"}); err == nil {
		p := "/search.json"
...
func (s *State) p2(w http.ResponseWriter, r *http.Request, rest string) {
	...
	if len(versions) == 0 {
		remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "composer"})
		if err == nil {
	...
	artifactkit.JSON(w, http.StatusOK, map[string]any{"packages": map[string]any{full: vs}})
}
...
func (s *State) versionEntry(ctx context.Context, name, version string) map[string]any {
	...
	entry := map[string]any{
		...
		"dist":    map[string]any{"type": "zip", "url": base + "/dist/" + name + "/" + artifactkit.URLencode(version) + "/" + version + ".zip", "shasum": ""},
...
func (s *State) providersAPI(w http.ResponseWriter, r *http.Request, rest string) {
	...
	if remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "composer"}); err == nil {
...
func storeVersionSource(reg *artifactkit.Registry, name, version string, data []byte, source string, ctx context.Context) {
	...
	if al := extractAutoload(data); al != nil {
		prop, _ = json.Marshal(map[string]any{"autoload": al})
	}
	reg.StoreVersion(ctx, artifactkit.VersionInput{
		Format: "composer", Repository: name, Version: version,
		Filename: short + "-" + version + ".zip", Source: source, Data: data,
		Proprietary: prop,
	})
}
