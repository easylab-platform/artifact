...
# skopeo refuses to run without a container signature policy; the lifecycle
# probe just re-tags public images, so accept everything.
mkdir -p "$HOME/.config/containers"
cat > "$HOME/.config/containers/policy.json" <<'X'
{"default": [{"type": "insecureAcceptAnything"}]}
X
SKOPEO="skopeo --policy $HOME/.config/containers/policy.json"
