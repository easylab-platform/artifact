...
func TestHostedRepodata(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPut, "/artifacts/conda/mychannel/linux-64/mytool-1.2.3-py310_0.conda", bytes.NewReader(pkg))
	s.ServeHTTP(rec, req)
	...
	s.ServeHTTP(rec2, httptest.NewRequest(http.MethodGet, "/artifacts/conda/mychannel/linux-64/repodata.json", nil))
	...
	s.ServeHTTP(rec3, httptest.NewRequest(http.MethodGet, "/artifacts/conda/mychannel/linux-64/mytool-1.2.3-py310_0.conda", nil))
