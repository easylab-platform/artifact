...
func (s *State) search(w http.ResponseWriter, r *http.Request) {
	...
	if remote, _ := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "npm"}); remote != nil {
		p := "/-/v1/search"
...
func (s *State) metadata(w http.ResponseWriter, r *http.Request, name string) {
	...
	remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "npm"})
	if err != nil {
...
func (s *State) aggregateMetadata(r *http.Request, name string) string {
	...
	if remote, err := s.Registry.Remote(r.Context(), artifactkit.UpstreamSpec{Format: "npm"}); err == nil {
